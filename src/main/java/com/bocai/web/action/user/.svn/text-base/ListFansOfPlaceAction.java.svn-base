...
@ParentPackage("user")
@Results({
	@Result(name="success",location="/WEB-INF/page/component/pod_fans_of_place.jsp"),
	@Result(name = "ajax", type = "json", 
			params = {"includeProperties","bokePager.*"}
	)
})
public class ListFansOfPlaceAction extends BaseAction<Object> {
	...
	@Action("list_fans_of_place")
	public String list(){
		if(id!=null && id!=0L){
			bokePager = placeManger.getFollowers(id, PAGE_SIZE, at);
		}
		return SUCCESS;
	}
	
	@Action("ajax_list_fans_of_place")
	public String ajax(){
		if(id!=null && id!=0L){
			bokePager = placeManger.getFollowers(id, PAGE_SIZE, at);
		}
