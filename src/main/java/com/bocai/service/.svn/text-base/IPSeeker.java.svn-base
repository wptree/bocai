...
import com.bocai.util.IPUtil;
import com.bocai.vo.IPEntry;
import com.bocai.vo.IPLocation;
...
public class IPSeeker {
    ...
    //纯真IP数据库名  
    private String ipFileName="QQWry.Dat";  

	//保存的文件夹  
    private String ipFileDirectory="";
    
    private String ipRestURL = "http://api.liqwei.com/location/";
    ...
    public IPSeeker(String fileName,String dir)  {  
        this.ipFileName=fileName;  
        this.ipFileDirectory=dir;  
        ...
        b3 = new byte[3];  
        try {  
            ipFile = new RandomAccessFile(ipFileName, "r");  
        } catch (FileNotFoundException e) {  
            // 如果找不到这个文件，再尝试再当前目录下搜索，这次全部改用小写文件名  
            // 因为有些系统可能区分大小写导致找不到ip地址信息文件  
            String filename = new File(ipFileName).getName().toLowerCase();  
            File[] files = new File(ipFileDirectory).listFiles(); 
            if(files!=null){
    ...
    private Map<String, String> geoByIp(String ip){
    	Map<String, String> map = new HashMap<String, String>();
    	try {
			URL url = new URL(ipRestURL);
			...
			inputReader.close();
			inputStream.close();
			logger.info("User from "+ sb.toString());
			String[] array = sb.toString().split(",");
			sb = null;
			
			map.put("ip", array[0]);
			map.put("country", array[1]);
			map.put("province", array[2]);
			map.put("city", array[3]);
			map.put("area", array[4]);
		} catch (Exception e) {
			logger.warn("IPSeeker could not get loaction from thirdparty rest api due to "+ e.toString());
			logger.info("Will change to local ip database ");
			map.put("ip", ip);
			map.put("city", getCountry(ip));
			map.put("area", getArea(ip));
		} 
		return map;
    }
    
    public String getCity(String ip){
    	Map<String, String> map = geoByIp(ip);
    	return map.get("city");
    }
    
    public String jsonLocation(String ip){
	...
	public String getIpFileDirectory() {
		return ipFileDirectory;
	}

	public void setIpFileDirectory(String ipFileDirectory) {
		this.ipFileDirectory = ipFileDirectory;
	}

	public String getIpRestURL() {
		return ipRestURL;
	}

	public void setIpRestURL(String ipRestURL) {
		this.ipRestURL = ipRestURL;
	}
}  