package com.bocai.common.hibernate;
...
import com.bocai.util.MyBeanUtils;
...
public abstract class HibernateSimpleDao {
	...
	protected void update(String hql, Object... values) {
	...
	/**
	 * 通过Finder获得分页数据，根据条件返回有序的@LinkedList
	 * 
	 * @param finder
	 *            Finder对象
	 * @param pageNo
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected Pagination findWithOrder(Finder finder, int pageNo, int pageSize) {
		int totalCount = countQueryResult(finder);
		Pagination p = new Pagination(pageNo, pageSize, totalCount);
		if (totalCount < 1) {
			p.setList(new ArrayList());
