...
import com.bocai.common.image.ImageUtils.Position;
...
public class CommonImageScale {
	...
	public static void resizeFix(File srcFile, File destFile, int boxWidth,
			...
			throws IOException {
		BufferedImage srcImgBuff = ImageIO.read(srcFile);
		srcImgBuff = srcImgBuff.getSubimage(cutLeft,cutTop, cutWidth, catHeight);
		int width = srcImgBuff.getWidth();
		int height = srcImgBuff.getHeight();
		if (width <= boxWidth && height <= boxHeight) {
	...
	public static void main(String[] args) throws Exception {
		...
		File srcFile = new File("C:\\Pictures\\Mr Tang_JPG.jpg");
		File destFile = new File("C:\\Pictures\\Mr Tang_JPG_new.jpg");
		info.setInput(new FileInputStream(srcFile));
		
		String mark = SystemConfig.getDomainUrl()+"/wptreetreere";
		System.out.println(mark.getBytes().length);
		int x = 0;
		if(mark.getBytes().length!=mark.length()){
			x = (mark.getBytes().length*10-100)*-1;
		}else{
			x = (mark.getBytes().length*10-30)*-1;
		}
		CommonImageScale.imageMark(srcFile, destFile, 400,400, 4, x, -20, mark, Color.WHITE, 14, 50);
